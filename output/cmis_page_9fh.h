/* Auto-generated by CMIS2C - CMIS Page 9Fh Register Map */
#ifndef __CMIS_PAGE_9FH_H
#define __CMIS_PAGE_9FH_H

#include <stdint.h>

/** @brief CMIS Page 9Fh Register Map (Bytes 128-255) */
typedef struct __attribute__((packed)) {

    /**
     * @brief CMDID
     * @details Query Status CMD ID
     */
    uint16_t CMDID;  /* r128 */
    /**
     * @brief EPLLength
     * @details EPL is not used
     */
    uint16_t EPLLength;  /* r130 */
    /**
     * @brief LPLLength
     * @details LPL length
     */
    uint8_t LPLLength;  /* r132 */
    /**
     * @brief CdbChkCode
     * @details Check Code over 9Fh:128-132 and LPL. See Table 8-178
     */
    uint8_t CdbChkCode;  /* r133 */
    /**
     * @brief RPLLength
     * @details U8 REPLY Payload Length (RPLLength) is computed by the module and encodes the length of REPLY data 
     * returned:0-120: Number of LPL bytes used in the LPL area 240≤i≤255: Number of EPL Pages used, encoded as i-239 
     * (PagesA0 to A0h+i-240) 121-239: Reserved The exact number of bytes returned in EPL Pages is determined in a 
     * CMDIDspecific way. RPLLength is computed in a CMDID specific way when data are returned in both LPL and EPL 
     * areas.Note: This REPLY header field is not included in the CdbChkCode computation. Hosts may want to set this 
     * byteto a suitable value when composing a CMD, in order to later check if the module has actually set this field 
     * inthe REPLY.
     * @access RW Cnd.
     */
    uint8_t RPLLength;  /* r134 */
    /**
     * @brief RPLChkCode
     * @details U8 REPLY Payload Check Code (RPLChkCode) is computed by the module as follows: No REPLY message body: 
     * RPLChkCode= 0 REPLY message body in Local Payload: RPLChkCode is the one’s complement of the sum of LPL Bytes 
     * 136to (136+RPLLength-1). REPLY message body in Extended Payload: RPLChkCode is the one’s complement of the sum 
     * ofall used bytes in EPL Pages A0h to (A0h+RPLLength-240). REPLY message body in LPL and EPL: RPLChkCode is 
     * computedas specified for the particular CMD. Note: This REPLY header field is not included in the CdbChkCode 
     * computation.Hosts may want to set this byte to a suitable value when composing a CMD, in order to later check if 
     * themodule has actually set this field in the REPLY.
     * @access RW Rqd.
     */
    uint8_t RPLChkCode;  /* r135 */
    /**
     * @brief LPL
     * @details Local Payload (120 bytes)
     */
    uint8_t LPL[120];  /* r136 */

} cmis_page_9fh_t;

#if __STDC_VERSION__ >= 202311L
static_assert(sizeof(cmis_page_9fh_t) == 128, "cmis_page_9fh_t must be exactly 128 bytes");
#elif __STDC_VERSION__ >= 201112L
_Static_assert(sizeof(cmis_page_9fh_t) == 128, "cmis_page_9fh_t must be exactly 128 bytes");
#elif defined(__GNUC__) || defined(__clang__)
_Static_assert(sizeof(cmis_page_9fh_t) == 128, "cmis_page_9fh_t must be exactly 128 bytes");
#else
/* verify manually: sizeof(cmis_page_9fh_t) must be 128 */
#endif
